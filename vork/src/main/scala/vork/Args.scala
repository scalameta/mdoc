...
object Args {
  def default(cwd: AbsolutePath): Args = new Args(
    in = cwd.resolve("docs"),
    out = cwd.resolve("out"),
    cwd = cwd
  )
  def fromCliArgs(args: List[String], logger: Logger, base: Args): Configured[Context] = {
    Conf
      .parseCliArgs[Args](args)
      .andThen(_.as[Args](decoder(base)))
      .andThen(_.validate(logger))
  }
  implicit val surface: Surface[Args] =
    generic.deriveSurface[Args]
  def decoder(base: Args): ConfDecoder[Args] = {
    ...
    generic.deriveDecoder[Args](base)
  }
